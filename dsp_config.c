...
static const char *_key(const char *json,const char *key){char needle[80];const char*p;snprintf(needle,sizeof(needle),"\"%s\"",key);p=strstr(json,needle);if(!p)return NULL;p+=strlen(needle);while(*p&&strchr(" \t\r\n",*p))p++;if(*p++!=':')return NULL;while(*p&&strchr(" \t\r\n",*p))p++;return p;}
static bool _double(const char*json,const char*key,double*v){const char*p=_key(json,key);char*end;if(!p)return false;errno=0;*v=strtod(p,&end);return !errno&&end!=p&&isfinite(*v);}
static bool _string(const char*json,const char*key,char*out,size_t cap){const char*p=_key(json,key),*e;size_t n;if(!p||*p!='\"')return false;e=strchr(++p,'\"');if(!e||(n=(size_t)(e-p))>=cap||memchr(p,'\\',n))return false;memcpy(out,p,n);out[n]='\0';return true;}
...
static bool _bool(const char*json,const char*key,bool*v){const char*p=_key(json,key);if(!p)return false;if(!strncmp(p,"true",4)){*v=true;return true;}if(!strncmp(p,"false",5)){*v=false;return true;}return false;}
...
char *dsp_config_load_options(const char *path,const char *expected_player) {
	...
	p=_key(json,"headroom_db");if(!p){free(json);free(out);return NULL;}if(!strncmp(p,"null",4))strcat(out,"headroom=auto;");else{char*end;errno=0;gain=strtod(p,&end);if(errno||end==p||!isfinite(gain)||gain<0||gain>60){free(json);free(out);return NULL;}snprintf(headroom,sizeof(headroom),"headroom=%.8g;",gain);strcat(out,headroom);}
	p=_key(json,"bypass");if(!p||(!_append(out,OPTION_LIMIT,"bypass=%s;",!strncmp(p,"true",4)?"true":!strncmp(p,"false",5)?"false":"INVALID"))||strstr(out,"INVALID")){free(json);free(out);return NULL;}
